...
impl I3Status {
    pub async fn new() -> Self {
        ...
        let stdout = i3status_process
            .stdout
            .take()
            .expect("Could not grab output from i3status");
        ...
        let header: serde_json::Value =
            serde_json::from_str(&header_string).expect("Could not get the header of i3status");
    ...
    pub async fn start_read_loop(&mut self, channel: &mut Channel) {
        loop {
            ...
            let values: Vec<serde_json::Value> =
                serde_json::from_str(&string[start_index..string.len()]).unwrap();
